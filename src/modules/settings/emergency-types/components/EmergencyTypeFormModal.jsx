...
function EmergencyTypeFormModal({ open, initialValues, loading, onCancel, onSubmit }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const message = normalizeApiError(err, "No fue posible guardar el tipo de emergencia.");
      setError(message);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/45 p-4 sm:items-center">
      <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl">
        <h3 className="text-lg font-extrabold text-slate-900">
          {isEditing ? "Editar tipo de emergencia" : "Nuevo tipo de emergencia"}
        </h3>
        ...
        <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
          ...
          <label className="block">
            <span className="mb-1.5 block text-sm font-semibold text-slate-700">Nivel</span>
            <select
              value={level}
...
export default EmergencyTypeFormModal;
