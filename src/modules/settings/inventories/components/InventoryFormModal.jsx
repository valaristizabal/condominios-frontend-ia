...
function InventoryFormModal({ open, initialValues, loading, onCancel, onSubmit }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const message = normalizeApiError(err, "No fue posible guardar la ubicación de inventario.");
      setError(message);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/45 p-4 sm:items-center">
      <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl">
        <h3 className="text-lg font-extrabold text-slate-900">
          {isEditing ? "Editar ubicación de inventario" : "Nueva ubicación de inventario"}
        </h3>
